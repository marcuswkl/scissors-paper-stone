...
object MyConfiguration {
  //any customisation of config must initialise localAddress and runlocalonly property
  var localAddress: Option[InetAddress] = None
  var runLocalOnly: Option[Boolean] = None

  def askDevConfig(): Config = {
    // Update the ipRaw address to your local IP address or Hamachi network IP address
